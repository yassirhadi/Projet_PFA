...
public interface AnalyseIARepository extends JpaRepository<AnalyseIA, Long> {
    ...
    @Query(value = "DELETE FROM analyse_ia WHERE offre_emploi_id = :offreId", nativeQuery = true)
    int deleteAllForOffreEmploiId(@Param("offreId") Long offreId);

    @Modifying(clearAutomatically = true, flushAutomatically = false)
    @Query(value = "DELETE FROM analyse_ia WHERE offre_privee_id = :offreId", nativeQuery = true)
