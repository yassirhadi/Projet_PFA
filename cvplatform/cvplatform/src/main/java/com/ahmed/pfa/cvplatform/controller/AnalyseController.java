...
import com.ahmed.pfa.cvplatform.dto.AnalysePriveeRequest;
...
public class AnalyseController extends BaseController {
    ...
    /**
     Lancer une analyse privée
     POST /api/analyses/privee
     */
    @PostMapping("/privee")
    public ResponseEntity<AnalyseResponse> lancerAnalysePrivee(
            @RequestBody AnalysePriveeRequest request) {
        AnalyseResponse response = analyseService.lancerAnalysePrivee(
                request.getCvId(),
                request.getOffrePriveeId()
        );
        return ResponseEntity.ok(response);
    }
}